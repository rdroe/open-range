...
- **Wheel** still drives horizontal scroll in JS; `lastWheelTime` marks **wheel-driven** scroll so a **range shift is not deferred** when the user might also be holding the mouse in the **content** area.
- **Pointer down** in the **swimlane / content** area (mouse / touch / pen) **defers** applying a **sector** shift until **pointerup** / **pointercancel**, so the domain remap does not run mid–content pan. A **pointer in the native scrollbar gutter** (bottom or right when that axis overflows) is **not** a content hold: the debounced **fetch + pack** (ticks + `mockData2` gap fill) can run as soon as the user reaches the band. When the build finishes, **layout paint and scroll-thumb compensation** wait until the last such pointer **releases** (so a long synthetic delay or slow ticks can complete while the thumb is still down, and the one-frame remap applies on **pointerup**).
- **Rebuild in flight** (`syntheticDelayMs` on mock fetch, ticks, and pack) **does not** use `pointer-events: none` on the aperture. The user can still **wheel** to pan; only a **new** sector shift is deferred (and the **draw** of updated lanes lags the async work). A sector shift’s scroll compensation after `await` uses the **current** `scrollLeft`, so panning during a long delay is not discarded.
- **Wipe mock session & regenerate** (`data-testid="scroll-lanes-wipe-mock"`) clears the tag-matched `mockData2` store, awaits a full `fetchRange` + pack, then repaints. You can still **pan** during the wait; **scroll position** is whatever the user has when the run finishes, then `suppressScroll` + `clamp` as needed. View settings are re-saved to localStorage.
